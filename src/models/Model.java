...
public class Model {
    ...
    public void askPlayerName() {
        ...
        // Kui mängija loobub nime sisestamisest (vajutades tühistamisnuppu), siis playerName muutub null-iks
        if (playerName == null) {
            // Siin võiks teha midagi vastavalt teie ärireeglitele,
            // näiteks määrata mingi vaikimisi nimi või lihtsalt jätkata ilma nime seadmata
            playerName = "Anonymous"; // Näide vaikimisi nime määramisest
        }

        // Kontrollime, kas mängija on nime sisestanud ja see pole tühi
        else if (playerName.isEmpty() || playerName.trim().length() < 2) {
            // Kui nimi on tühi või liiga lühike, küsime uuesti
            askPlayerName();
        }
    }
